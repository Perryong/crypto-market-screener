...
class Database:
    ...
    def insert_trade(self, exchange: str, symbol: str, price: float, qty: float,
        ...
        if not all(math.isfinite(v) and v > 0 for v in (price, qty, quote_qty)):
            return
        self.conn.execute("""
    ...
    def get_footprint_candles(self, exchange: str, symbol: str, interval_ms: int,
        ...
        result = self.conn.execute("""
            SELECT
                ...
                FLOOR(ROUND(price / ?, 8)) * ? as price_level,
                MIN(price) as low,
                MAX(price) as high,
        ...
        for row in result:
            ...
            decimals = max(1, -Decimal(str(tick_size)).normalize().as_tuple().exponent)
            price_key = f"{price_level:.{decimals}f}"
    ...
    def get_orderbook_heatmap_historical(self, exchange: str, symbol: str,
        ...
        # Use the finest compatible resolution per timestamp; never sum duplicate resolutions.
        result = self.conn.execute("""
            WITH usable AS (
                SELECT *, MIN(tick_size) OVER (PARTITION BY timestamp) AS finest
                FROM orderbook_snapshots
                WHERE exchange = ? AND symbol = ? AND timestamp >= ? AND tick_size <= ?
                  AND ABS(? / tick_size - ROUND(? / tick_size)) < 0.000001
            )
            SELECT timestamp, price, bid_qty, ask_qty FROM usable WHERE tick_size = finest
            ORDER BY timestamp, price
        """, [exchange, symbol, start_time, tick_size, tick_size, tick_size]).fetchall()
        ...
        for snap in snapshots:
            for side in ('bids', 'asks'):
                ...
                for level in snap[side]:
                    ratio = round(level['price'] / tick_size, 8)
                    price = (math.floor(ratio) if side == 'bids' else math.ceil(ratio)) * tick_size
                    levels[price] = levels.get(price, 0) + level['quantity']
                snap[side] = [{'price': p, 'quantity': q} for p,q in sorted(levels.items(), reverse=side=='bids')]
            bucket = snap['timestamp'] // interval_ms * interval_ms
            grouped[bucket] = {**snap, 'timestamp': bucket}
